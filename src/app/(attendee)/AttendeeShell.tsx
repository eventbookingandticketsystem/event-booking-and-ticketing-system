...
interface SidebarItem { id: AttendeeTab; label: string; icon: keyof typeof icons; }
...
const TAB_LABELS: Record<AttendeeTab, string> = {
  home: "Home", discover: "Discover", tickets: "My Tickets", account: "Account",
};

function AttendeeSidebar({ active, onNav }: { active: AttendeeTab; onNav: (tab: AttendeeTab) => void }) {
  ...
  return (
    <aside className="flex flex-col w-[220px] min-h-screen bg-brand-navy shrink-0">
      ...
      <div className="px-3 pb-4 flex flex-col gap-0.5 border-t border-white/10 pt-4">
        <button type="button" onClick={() => router.push(ROUTES.HOME)} aria-label="Back to home page"
        ...
        <button type="button" onClick={() => signOut({ redirect: true, callbackUrl: "/login" })} aria-label="Sign out"
...
function AttendeeTopbar({ active }: { active: AttendeeTab }) {
  ...
  return (
    <header className="flex items-center justify-between px-6 h-14 border-b border-border bg-surface shrink-0">
      <span className="font-display font-semibold text-[17px] text-text">{TAB_LABELS[active]}</span>
      <div className="flex items-center gap-3">
        ...
        <button type="button" onClick={() => router.push(ROUTES.ACCOUNT)} aria-label="Go to account settings"
          ...
          {image
            ...
            : <div className="w-full h-full bg-brand-navy inline-flex items-center justify-center text-[13px] font-bold text-white font-body">{initials}</div>
          }
        </button>
      </div>
    </header>
...
export default function AttendeeShell({ children }: { children: React.ReactNode }) {
  ...
  const handleTab = (tab: AttendeeTab) => router.push(tabRoute(tab));
