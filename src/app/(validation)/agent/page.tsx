...
const GATE_EVENTS = [
  ...
  { id: "evt-grad",   name: "University of Juba Graduation 2025", tickets: 1200, date: "Fri, 5 Dec 2025" },
  { id: "evt-tech",   name: "Juba Tech & Innovation Summit",   tickets: 400,  date: "Wed, 17 Dec 2025" },
  { id: "evt-gospel", name: "Juba Gospel Night",               tickets: 3250, date: "Sat, 28 Dec 2025" },
];
...
export default function GateSelectorPage() {
  ...
  const [selectedId, setSelectedId] = useState(GATE_EVENTS[0].id);
  const [dropOpen,   setDropOpen]   = useState(false);
  const [phase,      setPhase]      = useState<Phase>("downloading");
  const [pct,        setPct]        = useState(0);
  const dropRef = useRef<HTMLDivElement>(null);

  const ev = GATE_EVENTS.find((e) => e.id === selectedId) ?? GATE_EVENTS[0];
  ...
  const selectEvent = (id: string) => {
    setSelectedId(id);
    setDropOpen(false);
    // Re-trigger download for the new event
    ...
    <div className="h-full bg-brand-navy flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center p-4 overflow-y-auto">
        <div className="w-full max-w-[390px] flex flex-col gap-6 p-8 rounded-2xl bg-white/5 backdrop-blur-sm">
          ...
          {/* 3. Event selector — custom dropdown */}
                      ...
                      >
                        <div className="flex flex-col gap-[3px] min-w-0">
                          ...
                          <span className="text-[11px] text-white/40">
                            {option.date} · {option.tickets.toLocaleString()} tickets
                          </span>
                        </div>
                        {isSelected && (
                          <Icon name="Check" size={16} className="text-brand-orange shrink-0 mt-0.5" />
                        )}
                      </button>
                    );
                  })}
